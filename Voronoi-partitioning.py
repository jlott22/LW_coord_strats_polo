...
GRID_SIZE = 19  # grid is GRID_SIZE x GRID_SIZE intersections
...
# --- Voronoi partitioning weights ---
VORONOI_ALPHA = 1.0   # weight on distance term
VORONOI_BETA  = 2.0   # global strength of clue influence in Voronoi
...
# Partitions: list of regions; each region is a set of cell indices
partitions = None
my_region_index = None   # which region this robot currently owns
voronoi_robot_order = []  # robot_id list matching partitions indices
    ...
    def select_candidate(allow_searched: bool):
        ...
        for y in range(GRID_SIZE):
            for x in range(GRID_SIZE):
                ...
                # Restrict to our Voronoi region if defined
                if region_cells is not None and i not in region_cells:
                    continue
...
def compute_partitions():
    """
    Voronoi-style partitions over the grid:
      - one region per robot in sync_positions
      - cost C_i(c) = VORONOI_ALPHA * dist(robot_i, c)
                      - beta_i * P_clue(c)
      - beta_i = VORONOI_BETA * sum_k 1/(dist(robot_i, clue_k) + 1)

    Returns:
        (regions, robot_ids)
        regions: list[set[int]] of cell indices owned by each robot
        robot_ids: list of robot_ids in the same order as regions
    """
    global sync_positions, clues, clue_p

    if not sync_positions:
        # No sync info; no meaningful partition
        return None, []

    robot_ids = sorted(sync_positions.keys())

    # --- Compute beta_i for each robot (sum of influences from all clues) ---
    beta = {}
    if clues:
        for rid in robot_ids:
            rx, ry = sync_positions[rid]
            influence_sum = 0.0
            for (cx, cy) in clues:
                dist_rc = manhattan(rx, ry, cx, cy)
                influence_sum += 1.0 / (dist_rc + 1)
            beta[rid] = VORONOI_BETA * influence_sum
    else:
        # Pre-clue: pure distance Voronoi
        for rid in robot_ids:
            beta[rid] = 0.0

    # --- Build regions: assign each free cell to robot with min cost ---
    regions = [set() for _ in robot_ids]
    ...
    for y in range(GRID_SIZE):
        for x in range(GRID_SIZE):
            ...
            for k, rid in enumerate(robot_ids):
                rx, ry = sync_positions[rid]
                d = manhattan(rx, ry, x, y)
                cost = VORONOI_ALPHA * d - beta[rid] * p_clue
                if best_cost is None or cost < best_cost:
                    best_cost = cost
                    best_k = k

            regions[best_k].add(i)

    return regions, robot_ids

def recompute_partitions_and_assign():
    """
    Called when we have sync_positions from all robots.
    Recompute Voronoi partitions and update my_region_index.
    """
    global partitions, my_region_index, voronoi_robot_order

    partitions, robot_order = compute_partitions()
    voronoi_robot_order = robot_order

    if robot_order and ROBOT_ID in robot_order:
        my_region_index = robot_order.index(ROBOT_ID)
    else:
        my_region_index = None
...
def a_star(start, goal):
    ...
    while frontier and running and not found_target:
        ...
        for dx, dy in DIRS4:
            ...
            if partitions is not None and my_region_index is not None and 0 <= my_region_index < len(partitions):
                region_cells = partitions[my_region_index]
                current_in = idx(cx, cy) in region_cells
                next_in = idx(nx, ny) in region_cells
